...
std::vector<uint8_t> decode_mnemonic(const word_list& mnemonic, language lang, const std::string& password /* = "" */) {
    ...
    for (const auto& word : mnemonic)
    {
        ...
        if (position == -1) {
            throw std::runtime_error("wrong word from passphrase");
        }
...
bool valid_mnemonic(const word_list& mnemonic, language lang /* = language::en */) {
    try {
        auto data = decode_mnemonic(mnemonic, lang, "");
