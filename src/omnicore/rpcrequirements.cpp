...
{
    if ((0.01 * COIN) < amount) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Reference amount higher is than 0.01 BHD1");
    }
}
...
void RequireSaneDExFee(const std::string& address, uint32_t propertyId)
{
    ...
    if (poffer->getMinFee() > 1000000) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Minimum accept fee is higher than 0.01 BHD1 (use override = true to continue)");
    }
}
