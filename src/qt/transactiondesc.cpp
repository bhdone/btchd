...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord *rec, int unit)
{
    ...
    {
        strHTML += "<b>" + tr("Status") + ":</b> " + tr("This binding has been unable to mine") + " (" + FormatTxStatus(wtx, status, inMempool, numBlocks) + ")";
    }
    else
    {
    ...
    // Message
    //
    if (wtx.value_map.count("tx_text") && !wtx.value_map["tx_text"].empty())
        strHTML += "<br><b>" + tr("Message") + ":</b><br>@@" + GUIUtil::HtmlEscape(HexStr(wtx.value_map["tx_text"]), true) + "<br><br>";
    ...
    if (rec->type == TransactionRecord::BindPlotter && rec->status.status != TransactionStatus::Disabled)
    {
        ...
        if (IsValidPlotterID(wtx.value_map["plotter_id"], &plotterId)) {
            const COutPoint coinEntry(wtx.tx->GetHash(), 0);
            const Coin &bindCoin = wallet.chain().accessCoin(coinEntry);
            if (!bindCoin.IsSpent() && bindCoin.IsBindPlotter()) {
                int nSpendHeight = wallet.chain().lock()->getHeight().get_value_or(0) + 1;
                int nActiveHeight = wallet.chain().getUnbindPlotterLimitHeight(CBindPlotterInfo(coinEntry, bindCoin));
                if (nActiveHeight > nSpendHeight) {
