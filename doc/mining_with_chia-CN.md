...
当配置文件填写完成后，请使用命令`src/btchd-miner account --bind`来尝试绑定Farmer信息到BHD链上。

## 启动挖矿程序

1. 先确保已经启动了BHD的节点，并打开RPC消息模式。可以使用命令`src/btchdd -server`，或是测试网络则添加参数`-testnet`。
